...
// ============================================================================
// Trusted Partner Actions (for home page logo marquee)
// ============================================================================

/**
 * Get trusted partners for home page display
 * @param limit - Number of partners to fetch (default: 50)
 */
export async function getTrustedPartners(
  limit: number = 50,
): Promise<TrustedPartner[]> {
  try {
    const { data: partners } = await sanityFetch({
      query: `*[_type == "trustedPartner"] | order(order asc, _createdAt desc) {
        _id,
        companyName,
        logo
      }[0...${limit}]`,
    ...
    return partners.map(
      (partner: { _id: string; companyName: string; logo?: any }) => ({
        id: partner._id,
