  ...
  return (
    <section className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-4">
        <div className="h-12 bg-muted rounded-md w-[400px] animate-pulse" />
        ...
        {SKELETON_COLUMNS.map((column) => (
          <div key={column.id} className="flex flex-col gap-6">
            {column.items.map((id) => (
              <div
                key={id}
                className="flex flex-col gap-3 p-5 rounded-[20px] bg-muted animate-pulse"
              >
            ...
            {column.hasHighlight && (
              <div className="flex flex-col items-start justify-center p-5 rounded-[20px] h-[156px] bg-muted animate-pulse">
                <div className="h-7 bg-muted-foreground/20 rounded-md w-3/4" />
              </div>
            )}
          </div>
