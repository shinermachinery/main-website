...
import { fetchCompleteHomePageData } from '@/actions'

export async function HomePageDataExample() {
  // Fetch all home page data in a single request
  const { home, featuredProducts, teamMembers, testimonials } = await fetchCompleteHomePageData()

  if (!home) {
    return (
      <div className="text-center py-12">
        <p className="text-zinc-500">Home page content not available</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-12">
        ...
        <section className="space-y-4">
          ...
          {home.wordAboutUsDescription && (
            <div className="prose">
              ...
              <p className="text-zinc-600">Content would be rendered here...</p>
            </div>
          )}
        </section>
      )}
      ...
      {home.facts && home.facts.length > 0 && (
        <section className="space-y-4">
          <h2 className="text-2xl font-bold">{home.fewMoreFactsTitle || 'Few More Facts'}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {home.facts.map((fact) => (
              <div key={fact._key} className="text-center">
                <div className="text-4xl font-bold text-blue-600">{fact.number}</div>
                <div className="text-zinc-600 mt-2">{fact.text}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredProducts.map((product) => (
              <div key={product._id} className="border border-zinc-200 rounded-lg p-4">
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {testimonials.map((testimonial) => (
              <div key={testimonial._id} className="border border-zinc-200 rounded-lg p-4">
                <p className="text-zinc-600 italic">&ldquo;{testimonial.content}&rdquo;</p>
                <div className="mt-4">
